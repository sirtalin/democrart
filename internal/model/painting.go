...
type Painting struct {
	...
	Genres       []*Genre       `gorm:"many2many:painting_genres;association_autocreate:false"`
	Styles       []*ArtMovement `gorm:"many2many:painting_styles;association_autocreate:false"`
	Medias       []*Media       `gorm:"many2many:painting_medias;association_autocreate:false"`
...
type Genre struct {
	...
	Paintings []*Painting `gorm:"many2many:painting_genres"`
}

type Media struct {
	gorm.Model
	Name      string      `gorm:"type:varchar(32);not null;unique" json:"name"`
	Paintings []*Painting `gorm:"many2many:painting_medias"`
}
...
type PaintingMedia struct {
	PaintingID uint      `sql:"type:int REFERENCES paintings(id)" json:"painting_id"`
	Painting   *Painting `gorm:"foreignkey:id"`
	MediaID    uint      `sql:"type:int REFERENCES medias(id)" json:"media_id"`
